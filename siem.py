...
def write_json_format(results, config):
    ...
    for i in results:
        ...
        print(f"{i['datastream'].lower()}...................................")
        if i["datastream"].lower() =="event":
            print(f"\n\ninserint{i}")
            Eventinserter(conn=config.conn, **i)
        if i["datastream"].lower() =="alert":
            pass
...
SQLEventInsert = ("INSERT INTO sophos_test.dbo.[Event] ([source_info], [customer_id], [type], [id], [group], [name], [datastream], [rt], [end], [dhost], [severity]) VALUES (?,?,?,?,?,?,?,?,?,?,?);") 
SQLAlertInsert = "(INSERT INTO sophos_test.dbo.[ALERT]() VALUES ();)"
...
def Eventinserter(conn, **Values):
 """Values = ([source_info], [customer_id], [type], [id], [group], [name], [datastream], [rt], [end], [dhost], [severity])"""
 for item in Values:
    Values[item] =""
    if isinstance(Values[item], dict) and len(Values[item]) ==0:
        print("\n\nreplace empty object with empty string................")
 Values2 = (Values["source_info"], Values["customer_id"], 
            Values["type"], Values["id"], 
            Values["group"], Values["name"], 
            Values["datastream"], 
            Values["rt"], Values["end"], 
            Values["dhost"], Values["severity"]
            )
 if conn.closed == True:
  conn = connect()
 cursor = conn.cursor()	
 cursor.execute(SQLEventInsert, Values2)
 conn.commit()
 print(f"Insertion Done......................")
 
def Aleertinserter(conn, **Values):
 """Values = ()"""
 for item in Values:
    Values[item] =""
    if isinstance(Values[item], dict) and len(Values[item]) ==0:
...
def main():
    ...
    print('Driver={SQL Server Native Client 11.0};'+
                  ...
                  f'Database={config_data.db_name};'+
                  'Trusted_Connection=yes;')    
    conn = connect(config_data)
    config_data.conn = conn
